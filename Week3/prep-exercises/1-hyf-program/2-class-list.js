  ...
  const mentorInClass = mentors.filter(mentor => mentor.nowTeaching === classInfo.currentModule)
                               .map(mentor => ({ name: mentor.name, role: 'mentor' }));

  return [...studentsInClass, ...mentorInClass];
  ...
  const getActiveClasses = () => {
    ...
    const activeClassesWithPeople = {};
    activeClasses.forEach(classItem => {
      const peopleInClass = getPeopleOfClass(classItem.name);
      activeClassesWithPeople[classItem.name] = peopleInClass;
    });
